...
void setup(){
	P0DIR = 0xF7; //1111 0111 - Pinos do Port0
	...
	setup_T0_ticks(6666, 60006);	//Configura o timer como descrito:
																...
																//60 006 vezes é o maximo que pode repetir este tempo
																//4.9995 * 60 006 = 300 segundos = 5 minutos
	prepare_T0(); //Ativa as interrupções e deixa o timer pronto para ser iniciado
	
	//Pisca os Leds Indicando que o Dispositivo foi iniciado
	LED = 1; delay_ms(10); LED = 0; delay_ms(10);
	LED = 1; delay_ms(10); LED = 0; delay_ms(0);
...
void avalia_comando(){
		switch(rx_buf[1]){
			...
			case DESLIGA_TIMER_CMD:
				...
				timer_count = 0;
				break;
			case LER_BTN_FLAG_CMD:
				delay_ms(10);
				tx_buf[0] = MY_SUB_ADDRESS;
				tx_buf[1] = LER_BTN_FLAG_CMD;
				tx_buf[2] = btn_passou;
				TX_Mode_NOACK(3);
				btn_passou = 0;
				LED=!LED;
	...
	while(1){		
		...
		if(newPayload){
			...
			if(rx_buf[0]==BROADCAST_ADDRESS){ //Se for uma mensagem para todo mundo
				avalia_comando();
			} else if(rx_buf[0]==MY_SUB_ADDRESS){ //Se for uma mensagem para todo mundo
				avalia_comando();
			}
		}
	}
